...
public class H2SubscriptionsRepository implements ISubscriptionsRepository {
    ...
    public void addNewSubscription(Subscription subscription) {
        subscriptions.put(subscription.getTopicFilterRewritten() + "-" + subscription.getClientId(), subscription);
    }

    @Override
    public void removeSubscription(Subscription subscription) {
        final String clientID = subscription.getClientId();
        final Topic topicFilter = subscription.getTopicFilterRewritten();
    ...
    public void removeSharedSubscription(Subscription sub) {
        ...
        final Topic topicFilter = sub.getTopicFilterRewritten();
        final String sharedSubsMapName = sharedSubscriptions.get(clientId);
        if (sharedSubsMapName == null) {
    ...
    public void addNewSharedSubscription(Subscription sub) {
        ...
        storeNewSharedSubscription(sub.getClientId(), sub.getShareName(), sub.getTopicFilterRewritten(), qosPart);
    }
    ...
    private static final class SubscriptionValueType extends BasicDataType<Subscription> {
        ...
        public int getMemory(Subscription sub) {
            return StringDataType.INSTANCE.getMemory(sub.getClientId()) +
                StringDataType.INSTANCE.getMemory(sub.getTopicFilterRewritten().toString()) +
        ...
        public void write(WriteBuffer buff, Subscription sub) {
            ...
            StringDataType.INSTANCE.write(buff, sub.getTopicFilterRewritten().toString());
