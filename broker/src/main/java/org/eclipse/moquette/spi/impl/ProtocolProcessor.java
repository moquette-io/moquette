...
import org.moquette.configurationmanager.plugins.ICustomPublishingNeeds;
...
class ProtocolProcessor implements EventHandler<ValueEvent> {
	...
	/**
	 * The interface that is responsible of doing extract job when receiving a
	 * message from a client
	 */
	private final ICustomPublishingNeeds customPublishingNeeds;

	/**
	 * Setting the interface dynamically in the run time
	 * 
	 * @param customPublishingNeeds
	 */
	@Inject
	ProtocolProcessor(ICustomPublishingNeeds customPublishingNeeds) {
		this.customPublishingNeeds = customPublishingNeeds;
	}

	// William: I deleted this because I added a new final internface and I have
	// to define it.
	// Thus, I created a new constructor
	// ProtocolProcessor() {}
	...
	void processPublish(ServerChannel session, PublishMessage msg) {
		...
		String clientID = (String) session
				.getAttribute(NettyChannel.ATTR_KEY_CLIENTID);
		// William, it was final String topic, and I removed the final keyword
		...
		if (m_authorizator.canWrite(topic, user, clientID)) {
			...
			if (!this.customPublishingNeeds
				...
				HashMap<String, String> newValues = (HashMap<String, String>) this.customPublishingNeeds
