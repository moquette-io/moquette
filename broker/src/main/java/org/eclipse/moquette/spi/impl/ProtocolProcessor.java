...
class ProtocolProcessor implements EventHandler<ValueEvent> {
	...
	/**
	 * the interface for the plugins responsible for the additional needs that
	 * could be required when a client publish a message and before the broker
	 * does the actual publishing of the message
	 * 
	 * @author williamkinaan
	 */
	private ICustomPublishingNeeds customPublishNeeds;

	ProtocolProcessor() {
		/**
		 * The constructor was empty and I added the following code to
		 * initialize the customPublishNeeds. Notice, there should never be any
		 * exception when initializing the interface because the plugin manager
		 * already validates the type of the classes and if the user provides
		 * any implementation for iCustomPublishingNeeds that is actually
		 * doesn't implement the ICustomPublishingNeeds, a default custom needs
		 * class will be used. Just if the developer deleted the default custom
		 * needs class, an exception will be thrown here and it will be logged
		 * as well.
		 * 
		 * @author williamkinaan
		 */
		try {
			this.customPublishNeeds = (ICustomPublishingNeeds) Class
					.forName(
							PluginConfigurationParser.plugins_configurations
	...
	void init(SubscriptionsStore subscriptions, IMessagesStore storageService,
		...
		Disruptor<ValueEvent> disruptor = new Disruptor<>(
				ValueEvent.EVENT_FACTORY, 1024 * 32, m_executor);
	...
	void processConnect(ServerChannel session, ConnectMessage msg) {
		...
		if (msg.getProcotolVersion() != VERSION_3_1
				&& msg.getProcotolVersion() != VERSION_3_1_1) {
		...
		LOG.info("Create persistent session for clientID <{}>",
				msg.getClientID());
		m_sessionsStore.addNewSubscription(Subscription
				.createEmptySubscription(msg.getClientID(), true)); // null
																	// means
																	// EmptySubscription
	...
	void processPublish(ServerChannel session, PublishMessage msg) {
		...
		if (m_authorizator.canWrite(topic, user, clientID)) {
			// William inside if (m_authorizator.canWrite(topic, user,
			// clientID)) { there was just executePublish(clientID, msg);
			if (!this.customPublishNeeds
					.isThereAditionalChecksBeforePublishing(topic, user,
							clientID)) {
				if (this.customPublishNeeds
						.shouldPublishTheMessageToAllClients(topic, user,
								clientID)) {
					executePublish(clientID, msg);
				}
			} else {
				CharBuffer charBuffer = StandardCharsets.US_ASCII.decode(msg
						.getPayload());
				String payLoadAsString = charBuffer.toString();
				HashMap<String, String> newValues = (HashMap<String, String>) this.customPublishNeeds
						.doAdditinalWorkBeforePublishing(topic, user, clientID,
								payLoadAsString);
				msg.setTopicName(newValues.get("topic"));
				user = newValues.get("user");
				clientID = newValues.get("clientID");
				msg.setPayload(ByteBuffer.wrap(newValues.get("payload")
	...
	private void executePublish(String clientID, PublishMessage msg) {
		...
		if (qos == AbstractMessage.QOSType.MOST_ONE) { // QoS0
			...
			// TODO use a message store for TO PUBLISH MESSAGES it has nothing
			// to do with inFlight!!
				...
				if (!sub.isCleanSession() && !sub.isActive()) {
					...
					if (qos == AbstractMessage.QOSType.EXACTLY_ONCE) {
						PublishEvent newPublishEvt = new PublishEvent(topic,
								...
								messageID != null ? messageID : 0);
						m_messagesStore.addInFlight(newPublishEvt,
	...
	private void sendPubRec(String clientID, int messageID) {
		...
		disruptorPublish(new OutputMessagingEvent(m_clientIDs.get(clientID)
	...
	void processDisconnect(ServerChannel session, DisconnectMessage msg)
